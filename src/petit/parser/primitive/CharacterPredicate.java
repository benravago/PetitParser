...
public interface CharacterPredicate {
  ...
  static CharacterPredicate anyOf(String string) {
    var ranges = string.chars().mapToObj(value -> new CharacterRange((char) value, (char) value)).collect(Collectors.toList());
  ...
  /**
   * Returns a character predicate that matches character ranges between {@code starts} and {@code stops}.
   */
  static CharacterPredicate ranges(char[] starts, char[] stops) {
    ...
    return value -> {
      var index = Arrays.binarySearch(starts, value);
  ...
  class PatternParser {
    static final PetitParser PATTERN_SIMPLE = CharacterParser.any().map((Character value) -> new CharacterRange(value, value));
    static final PetitParser PATTERN_RANGE = CharacterParser.any().seq(CharacterParser.of('-')).seq(CharacterParser.any()).map((List<Character> values) -> new CharacterRange(values.get(0), values.get(2)));
    static final PetitParser PATTERN_POSITIVE = PATTERN_RANGE.or(PATTERN_SIMPLE).star().map(CharacterRange::toCharacterPredicate);
    static final PetitParser PATTERN = CharacterParser.of('^').optional().seq(PATTERN_POSITIVE).map((List<CharacterPredicate> predicate) -> predicate.get(0) == null ? predicate.get(1) : predicate.get(1).not()).end();
  }
