 ...
 * <p>
 * The token holds the resulting value of the input, the input buffer, and the start and stop position in the input buffer.
 * It provides many convenience methods to access the state of the token.
 */
public class Token {
  ...
  public String toString() {
    var tuple = lineAndColumnOf(buffer, start);
   ...
   */
  public static final PetitParser NEWLINE_PARSER = of('\n').or(of('\r').seq(of('\n').optional()));
  ...
  public static int[] lineAndColumnOf(String buffer, int position) {
    var tokens = NEWLINE_PARSER.token().<Token>matchesSkipping(buffer);
    int line = 1, offset = 0;
